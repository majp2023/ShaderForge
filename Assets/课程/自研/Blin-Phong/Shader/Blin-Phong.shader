Shader "Shader/Blin-Phong"
{
    Properties {
        //TODO 材质面板参数
        _MainCol("基本色", color) = (1.0,1.0,1.0,1.0)
        _MainTexture ("RGB:主帖图 A:环境遮罩", 2D) = "white" {}
        _Cubemap    ("环境贴图", Cube) = "_Skybox" {}
        _NormalMap  ("法线贴图", 2D) = "bump" {}
        _SpecularPow("高光次幂", range(1, 90)) = 30
        _CubemapMip ("环境球Mip", Range(0, 7)) = 0
        _FresnelPow ("菲涅尔次幂", Range(0, 5)) = 1
        _EnvSpecInt ("环境镜面反射强度", Range(0, 5)) = 0.2
    }
    SubShader {
        Tags {
            "RenderType"="Opaque"
        }
        Pass {
            Name "FORWARD"
            Tags {
                "LightMode"="ForwardBase"
            }
            
            CGPROGRAM
            #pragma vertex vert
            #pragma fragment frag
            #include "UnityCG.cginc"
            #include"AutoLight.cginc"
            #include"Lighting.cginc"
            #pragma multi_compile_fwdbase_fullshadows
            #pragma target 3.0
            uniform float3 _MainCol;
            uniform sampler2D _MainTexture;
            uniform float _SpecularPow;
            uniform samplerCUBE _Cubemap;
            uniform sampler2D _NormalMap;
            uniform float _CubemapMip;
            uniform float _FresnelPow;
            uniform float _EnvSpecInt;
            struct VertexInput {
                //TODO 输入结构
                float4 vertex : POSITION;
                float2 uv : TEXCOORD0;
                float4 normal : NORMAL;
                float4 tangent  : TANGENT;      // 切线信息
            };
            struct VertexOutput {
                //TODO 输出结构
                float4 pos : SV_POSITION;
                float2 uv : TEXCOORD0;
                float4 posWS : TEXCOORD1;//世界空间顶点位置
                float3 nDirWS : TEXCOORD2;      // 世界法线方向
                float3 tDirWS : TEXCOORD3;      // 世界切线方向
                float3 bDirWS : TEXCOORD4;      // 世界副切线方向
                LIGHTING_COORDS(5,6)
            };
            VertexOutput vert (VertexInput v) {
                //TODO 顶点Shader
                VertexOutput o = (VertexOutput)0;
                o.pos = UnityObjectToClipPos( v.vertex );
                o.uv = v.uv;
                o.posWS = mul(unity_ObjectToWorld, v.vertex);// 顶点位置 OS>WS
                o.nDirWS = UnityObjectToWorldNormal(v.normal);  // 法线方向 OS>WS
                o.tDirWS = normalize(mul(unity_ObjectToWorld, float4(v.tangent.xyz, 0.0)).xyz); // 切线方向 OS>WS
                o.bDirWS = normalize(cross(o.nDirWS, o.tDirWS) * v.tangent.w);  // 根据nDir tDir求bDir
                TRANSFER_VERTEX_TO_FRAGMENT(o)
                return o;
            }
            float4 frag(VertexOutput i) : COLOR {
                //TODO 像素Shader
                //准备向量
                float nDir = normalize(i.posWS);//法线方向
                float lDir = _WorldSpaceLightPos0.xyz;//光方向
                float vDir = normalize(_WorldSpaceCameraPos.xyz - i.posWS.xyz);//视口方向
                float hDir = normalize(vDir + lDir);
                float3 nDirTS = UnpackNormal(tex2D(_NormalMap, i.uv)).rgb;//切线空间的世界坐标
                float3x3 TBN = float3x3(i.tDirWS, i.bDirWS, i.nDirWS);//TBN矩阵
                float3 nDirWS = normalize(mul(nDirTS, TBN));   // 计算Fresnel 计算vrDirWS
                float3 vrDirWS = reflect(-vDir, nDirWS);// 采样Cubemap

                //准备点积结果
                float halfLambort = dot(nDir, lDir) * 0.5 + 0.5;
                float nDoth = dot(nDir, hDir);
                float vdotn = dot(vDir, nDirWS);

                //贴图采样
                float4 mainTexture = tex2D(_MainTexture, i.uv);
                float blinnPhong = pow(max(0.0, nDoth), _SpecularPow);
                float3 var_Cubemap = texCUBElod(_Cubemap, float4(vrDirWS, _CubemapMip)).rgb;
                float fresnel = pow(max(0.0, 1.0 - vdotn), _FresnelPow);

                //最后结果
                float shadow = LIGHT_ATTENUATION(i);
                float3 envSpecLighting = var_Cubemap * fresnel * _EnvSpecInt * mainTexture.a;
                float3 finalRGB = halfLambort * _MainCol * mainTexture.rgb * shadow + envSpecLighting;//+ blinnPhong;

                float3 halfLambortMulMainCol = halfLambort * _MainCol * mainTexture.rgb + blinnPhong;

                return float4(halfLambortMulMainCol, 1.0);
                //return float4(finalRGB, 1.0);
            }
            ENDCG
        }
    }
    FallBack "Diffuse"
}
